...
<template>
  <BaseModal :open="open" size="detail" @close="$emit('close')">
    <div v-if="expense" class="detail-layout expense-detail-modern">
      ...
      <section class="surface-inline detail-section"><div class="section-label-row"><div><h3>تصمیم ارجاع گیرنده ها</h3></div><span class="meta-pill">{{ decisions.length }} نفر</span></div><div v-if="decisions.length" class="decision-list"><article v-for="item in decisions" :key="item.id" :class="['decision-row', item.status]"><div><strong>{{ item.approver }}</strong><small>{{ item.role }}</small></div><span>{{ item.statusLabel }}</span><p v-if="item.decisionNote">{{ item.decisionNote }}</p></article></div><div v-else class="empty-state-inline centered-empty"><span class="material-symbols-outlined">hourglass_empty</span><p>ارجاع گیرنده ای ثبت نشده است.</p></div></section>
      <section class="surface-inline detail-section"><div class="section-label-row"><div><h3>فاکتور</h3></div></div><button v-if="expense.invoiceUrl" class="action-btn tone-primary" type="button" @click="openProtectedFile(expense.invoiceUrl, expense.invoiceName || 'invoice')"><span class="material-symbols-outlined">description</span><span>مشاهده فایل</span></button><div v-else class="empty-state-inline centered-empty"><span class="material-symbols-outlined">description</span><p>فایل پیوست وارد نشده است.</p></div></section>
