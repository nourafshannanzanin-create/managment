...
const { canApproveSelectedRequest, approveSelectedRequest, rejectSelectedRequest, openProtectedFile, state } = useWorkflowHub()
  ...
  <BaseModal :open="open" size="detail" @close="$emit('close')">
    <div v-if="request" class="detail-layout request-detail-modern">
      ...
      <section class="surface-inline detail-section">
        ...
        <div v-else class="empty-state-inline"><span class="material-symbols-outlined">hourglass_empty</span><p>ارجاع گیرنده ای ثبت نشده است.</p></div>
      </section>
      ...
      <section class="surface-inline detail-section">
        ...
        <div v-else class="empty-state-inline"><span class="material-symbols-outlined">attach_file_off</span><p>فایلی ثبت نشده است.</p></div>
      </section>
...
@media (max-width: 760px) { .detail-summary-grid { grid-template-columns: 1fr; } .decision-row { grid-template-columns: 1fr; } }
</style>
